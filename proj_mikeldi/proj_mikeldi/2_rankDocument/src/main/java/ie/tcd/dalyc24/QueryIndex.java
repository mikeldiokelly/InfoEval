...
public class QueryIndex
{
	...
	public static void main(String[] args) throws IOException, ParseException
	{
		...
		FileWriter tfidfWriter = new FileWriter("../queries/tfidf_results.txt");
		FileWriter bmWriter = new FileWriter("../queries/bm25_results.txt");
		...
		do
		{
			...
			int query_index = 0;
			String queries = new String(Files.readAllBytes(Paths.get("../queries/queries.txt")));
			String[] qry = queries.split(".I");
			for (String q : qry) {
				int z = 0;
				String[] q2 = q.split(".W");
				for (String q3 : q2) {
					if (z == 1) {
						...
						if (queryString.length() > 0)
						{
							...
							for (int i=0; i<30; i++) {
								if (top_max[i] > 0) {
									tfidfWriter.write(query_index + " Q0 " + top_index[i] + " " + i + " " + top_max[i] + " STANDARD \n");
							...
							for (int i=0; i<30; i++) {
								if (top_max_BM[i] > 0) {
									bmWriter.write(query_index + " Q0 " + top_index_BM[i] + " " + i + " " + top_max_BM[i] + " STANDARD \n");
